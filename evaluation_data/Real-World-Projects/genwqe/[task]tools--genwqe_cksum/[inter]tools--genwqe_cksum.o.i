...
static void usage(const char *prog)
{
 printf("Usage: %s [-h] [-v, --verbose] [-C, --card <cardno>|RED]\n"
        "\t-C, --card <cardno> use cardno for operation (default 0)\n"
               "\t-A, --accel <GENWQE|CAPI> select accelerator type (CAPI only for ppc64le)\n"
        "\t-V, --version show software version\n"
        "\t-X, --cpu <cpu_number> only run on this CPU number\n"
        ...
        "\t-s, --bufsize <bufsize> default is %d KiB\n"
...
# 130 "genwqe_cksum.c"
 ...
 if (use_sglist)
  ats_type = 0x6ull;
 else ats_type = 0x4ull;
 cmd->ats = (((ats_type) & 0xf) << (44 - (4 * (
# 135 "genwqe_cksum.c" 3 4
           __builtin_offsetof (
# 135 "genwqe_cksum.c"
           struct asiv_memcpy
# 135 "genwqe_cksum.c" 3 4
...
# 135 "genwqe_cksum.c"
 ...
 if (use_sglist)
  ats_type = 0x7ull;
 else ats_type = 0x5ull;
 cmd->ats |= (((ats_type) & 0xf) << (44 - (4 * (
# 139 "genwqe_cksum.c" 3 4
            __builtin_offsetof (
# 139 "genwqe_cksum.c"
            struct asiv_memcpy
# 139 "genwqe_cksum.c" 3 4
...
# 139 "genwqe_cksum.c"
 ...
 if (verbose_flag) {
  fprintf(
# 142 "genwqe_cksum.c" 3 4
         stderr
# 142 "genwqe_cksum.c"
               , "ATS: 0x%llx use_sglist: %d\n", (long long)cmd->ats, use_sglist);
  fprintf(
# 143 "genwqe_cksum.c" 3 4
         stderr
# 143 "genwqe_cksum.c"
               , "Src:  %p\n", src);
  fprintf(
# 144 "genwqe_cksum.c" 3 4
         stderr
# 144 "genwqe_cksum.c"
               , "Dest: %p\n", dest);
  fprintf(
# 145 "genwqe_cksum.c" 3 4
         stderr
# 145 "genwqe_cksum.c"
               , "Len: 0x%x\n", (uint32_t)n);
 }
 ...
 if (verbose_flag > 1) {
  fprintf(
# 149 "genwqe_cksum.c" 3 4
         stderr
# 149 "genwqe_cksum.c"
               , "\n Dump Data @ %p\n", cmd);
  ddcb_hexdump(
# 150 "genwqe_cksum.c" 3 4
              stderr
# 150 "genwqe_cksum.c"
                    , cmd, sizeof(struct ddcb_cmd));
 }
 cmd->disp_ts = get_us();
 rc = accel_ddcb_execute(accel, cmd, 
# 153 "genwqe_cksum.c" 3 4
                                    ((void *)0)
# 153 "genwqe_cksum.c"
                                        , 
# 153 "genwqe_cksum.c" 3 4
...
# 153 "genwqe_cksum.c"
 ...
 if (verbose_flag > 1) {
  fprintf(
# 156 "genwqe_cksum.c" 3 4
         stderr
# 156 "genwqe_cksum.c"
               , "\n Dump Data @ %p\n", cmd);
  ddcb_hexdump(
# 157 "genwqe_cksum.c" 3 4
              stderr
# 157 "genwqe_cksum.c"
                    , cmd, sizeof(struct ddcb_cmd));
 }
...
# 163 "genwqe_cksum.c"
 ...
 if (verbose_flag)
  fprintf(
# 166 "genwqe_cksum.c" 3 4
         stderr
# 166 "genwqe_cksum.c"
               , "  crc32=%u adler32=%u inp_processed=%u in %lld usec\n",
                           ...
                           int use_adler)
{
 ...
 struct stat st;
 FILE *i_fp;
 uint32_t crc = 0, m_crc32 = 0;
 uint32_t m_adler32 = 1;
...
# 188 "genwqe_cksum.c"
 ...
 if (stat(in_f, &st) == -1) {
  fprintf(
# 192 "genwqe_cksum.c" 3 4
         stderr
# 192 "genwqe_cksum.c"
               , "err: stat on input file (%s)\n",
   strerror(
# 193 "genwqe_cksum.c" 3 4
           (*__errno_location ())
# 193 "genwqe_cksum.c"
                ));
   exit(79);
 ...
 if (!i_fp) {
  do { fprintf(
# 200 "genwqe_cksum.c" 3 4
 stderr
# 200 "genwqe_cksum.c"
 , "%s:%u: Error: " "err: can't open input file %s: %s\n",
                  "genwqe_cksum.c"
# 200 "genwqe_cksum.c"
  ,
                  201
...
# 200 "genwqe_cksum.c"
 ...
 while (size_f) {
  int tocopy = ({ __typeof__ (ibuf_size) _a = (ibuf_size); __typeof__ (size_f) _b = (size_f); _a < _b ? _a : _b; });

  rc = fread(ibuf, tocopy, 1, i_fp);
  if (rc != 1) {
   do { fprintf(
# 210 "genwqe_cksum.c" 3 4
  stderr
# 210 "genwqe_cksum.c"
  , "%s:%u: Error: " "err: can't read input file %s: %s\n",
                   "genwqe_cksum.c"
# 210 "genwqe_cksum.c"
   ,
                   211
...
# 210 "genwqe_cksum.c"
  ...
  rc = accel_cksum(accel, &cmd, ibuf, obuf, tocopy,
          &m_crc32, &m_adler32, &m_inp_processed,
          use_sglist);
  xerrno = 
# 221 "genwqe_cksum.c" 3 4
          (*__errno_location ())
# 221 "genwqe_cksum.c"
               ;
...
# 256 "genwqe_cksum.c"
   ...
   ddcb_hexdump(
# 258 "genwqe_cksum.c" 3 4
               stderr
# 258 "genwqe_cksum.c"
                     , cmd.asv, sizeof(cmd.asv));
   exit(
# 259 "genwqe_cksum.c" 3 4
       1
# 259 "genwqe_cksum.c"
                   );
  } else
   do { if (verbose_flag) fprintf(
# 261 "genwqe_cksum.c" 3 4
  stdout
# 261 "genwqe_cksum.c"
  , "  RETC: %03x %s ATTN: %x PROGR: %x\n" "  from card CRC32: %08x ADLER: %08x\n" "  DEQUEUE=%016llx CMPLT=%016llx DISP=%016llx\n", cmd.retc, retc_strerror(cmd.retc), cmd.attn, cmd.progress, m_crc32, m_adler32, (long long)cmd.deque_ts, (long long)cmd.cmplt_ts, (long long)cmd.disp_ts); } while (0)
 ...
 if ((check_result) && (m_crc32 != crc)) {
  fprintf(
# 279 "genwqe_cksum.c" 3 4
         stderr
# 279 "genwqe_cksum.c"
               , "err: CRCs do not match %u != %u\n",
   m_crc32, crc);
 }
...
# 293 "genwqe_cksum.c" 3 4
                                 _SC_PAGESIZE
# 293 "genwqe_cksum.c"
                                             );
 const char *in_f = 
# 294 "genwqe_cksum.c" 3 4
                   ((void *)0)
# 294 "genwqe_cksum.c"
                       ;
 int cpu = -1;
 int card_type = 0x0000;
 int check_result = 0;
 int use_sglist = 0;
 int use_adler = 0;
 int data_buf_size = (1 * 1024 * 1024);
...
# 339 "genwqe_cksum.c"
  ...
  case 'A':
   ...
   card_type = strtol(optarg, (char **)
# 351 "genwqe_cksum.c" 3 4
                                      ((void *)0)
# 351 "genwqe_cksum.c"
                                          , 0);
   if ((0x0000 != card_type) &&
...
# 355 "genwqe_cksum.c"
   ...
   break;
  case 'X':
   cpu = strtoul(optarg, (char **)
# 359 "genwqe_cksum.c" 3 4
                                 ((void *)0)
# 359 "genwqe_cksum.c"
                                     , 0);
   break;
...
# 389 "genwqe_cksum.c"
 ...
 if (-1 == card_no) {
  if (1 != use_sglist) {
   do { if (verbose_flag) fprintf(
# 398 "genwqe_cksum.c" 3 4
  stdout
# 398 "genwqe_cksum.c"
  , "I have to set Option -G set when in " "redundant card mode!\n"); } while (0)
                             ;
   use_sglist = 1;
  }
 }

 accel = accel_open(card_no, card_type, 0x0004 | 0x0008,
  &err_code, 0, 0x0000000000000000ull);
 if (accel == 
# 406 "genwqe_cksum.c" 3 4
             ((void *)0)
# 406 "genwqe_cksum.c"
                 ) {
...
# 409 "genwqe_cksum.c" 3 4
                                              (*__errno_location ())
# 409 "genwqe_cksum.c"
                                                   ));
  exit(
# 410 "genwqe_cksum.c" 3 4
      1
# 410 "genwqe_cksum.c"
                  );
 }
...
# 425 "genwqe_cksum.c"
  ...
  do { fprintf(
# 426 "genwqe_cksum.c" 3 4
 stderr
# 426 "genwqe_cksum.c"
 , "%s:%u: Error: " "cannot allocate memory\n", "genwqe_cksum.c", 426); } while (0);
  exit(
# 427 "genwqe_cksum.c" 3 4
      1
# 427 "genwqe_cksum.c"
                  );
 }
