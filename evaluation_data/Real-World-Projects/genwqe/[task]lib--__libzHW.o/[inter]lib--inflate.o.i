...
# 37 "../include/libzHW.h"
# 1 "/usr/lib/gcc/x86_64-linux-gnu/5/include/stdarg.h" 1 3 4
# 38 "../include/libzHW.h" 2

# 1 "../include/libddcb.h" 1
# 38 "../include/libddcb.h"
...
static void scratch_update(zedc_streamp strm)
{
 ...
 if (strm->avail_out > strm->outp_returned) {
...
static int post_scratch_upd(zedc_streamp strm)
{
 ...
 if ((strm->inp_processed == 0) && (strm->proc_bits == 0)) {
  ...
  if (strm->avail_in >
      ((0x0200) - strm->in_hdr_scratch_len)) {

   do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Error: " "scratch buffer too small\n", getpid(), gettid(), "inflate.c", 287); } while (0);
   zedc->zedc_rc = (-311);
   return zedc->zedc_rc;
  }
  if (strm->avail_in) {
...
# 307 "inflate.c"
 if ((strm->copyblock_len) &&
     ((strm->infl_stat & (0x20 | 0x40)) == 0) &&
     (strm->out_hdr_bits != 0)) {

  target = strm->wsp->tree;

  len = strm->copyblock_len;
  if (strm->infl_stat & 0x80)
   target[0] = 0x01;
  else
   target[0] = 0x00;
...
{
 uint16_t val16;
 head_state next_state = strm->header_state;
  ...
  case HEADER_START:
   if (strm->prefx_idx == 1) {
    val16 = ((uint16_t)strm->prefx[0] << 8) +
     strm->prefx[1];
    if ((val16 % 31) != 0) {
     do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Error: " "ZLIB header invalid (FCHECK)\n", getpid(), gettid(), "inflate.c", 375); } while (0);
     return 3;
    }

    if (((val16 & 0x0f00) != 0x0800) ||
     ((val16 & 0xf000) > 0x7000)) {
     do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Error: " "ZLIB header invalid (CMF)\n", getpid(), gettid(), "inflate.c", 381); } while (0);
     return 3;
    }
    if (val16 & 0x0020) {
...
# 740 "inflate.c"
    ...
    if (strm->file_adler32 != strm->adler32) {
     do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Error: " "ADLER32 mismatch: " "%08llx/%08llx\n", getpid(), gettid(),
...
# 744 "inflate.c"
     ,
                                    ...
                                    747
# 744 "inflate.c"
     , (long long)strm->file_adler32, (long long)strm->adler32); } while (0)
...
static int inflate_flush_output_buffer(struct zedc_stream_s *strm)
{
 ...
 if (strm->dict_len < strm->obytes_in_dict) {
  do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Error: " "invalid 'obytes_in_dict' ZEDC_ERR_DICT_OVERRUN\n", getpid(), gettid(), "inflate.c", 823); } while (0);
  zedc->zedc_rc = (-317);
  return zedc->zedc_rc;
 }
...
# 903 "inflate.c"
 ...
 if (len % 8ULL)
  do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Warn: " "[%s] in_hdr_scratch_len: 0x%llx not consistent " "(0x%x 0x%x 0x%x 0x%x)\n", getpid(), gettid(),
...
# 911 "inflate.c"
  ,
                                   ...
                                   917
# 911 "inflate.c"
  , __func__, (long long)len, (unsigned int)strm->tree_bits, (unsigned int)strm->pad_bits, (unsigned int)strm->scratch_ib, (unsigned int)strm->scratch_bits); } while (0)
 ...
 if (strm->in_hdr_scratch_len > (0x0200))
  do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Warn: " "[%s] in_scratch_len=%d exceeds ZEDC_TREE_LEN=%d\n", getpid(), gettid(),
                                                    "inflate.c"
# 924 "inflate.c"
  ,
                                                    925
# 924 "inflate.c"
  , __func__, strm->in_hdr_scratch_len, (0x0200)); } while (0)
                                                     ;
...
# 936 "inflate.c" 3 4
                 ((__be64)(__builtin_constant_p((__u64)((
# 936 "inflate.c"
                 (unsigned long)strm->wsp->dict[p] + strm->out_dict_offs
# 936 "inflate.c" 3 4
                 ))) ? ((__u64)( (((__u64)((
# 936 "inflate.c"
                 (unsigned long)strm->wsp->dict[p] + strm->out_dict_offs
...
# 938 "inflate.c" 3 4
                 ((__be64)(__builtin_constant_p((__u64)((
# 938 "inflate.c"
                 (unsigned long)strm->wsp->dict[p ^ 1]
# 938 "inflate.c" 3 4
                 ))) ? ((__u64)( (((__u64)((
# 938 "inflate.c"
                 (unsigned long)strm->wsp->dict[p ^ 1]
...
# 953 "inflate.c"
 ...
 do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Error: " "preserving %s %d bytes ...\n", getpid(), gettid(), "inflate.c", 959, fname, len); } while (0);

 fp = fopen(fname, "w+");
 if (!fp) {
  do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Error: " "Cannot open file %s: %s\n", getpid(), gettid(), "inflate.c", 963, fname, strerror(
# 963 "inflate.c" 3 4
 (*__errno_location ())
# 963 "inflate.c"
 )); } while (0);
 ...
 if (rc != 1) {
  do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Error: " "Cannot write all data: %d\n", getpid(), gettid(), "inflate.c", 968, rc); } while (0);
  fclose(fp);
  return (-1);
 }
 fclose(fp);
...
int zedc_inflate(zedc_streamp strm, int flush)
{
 ...
 if (strm->avail_in > (0xffffffff - 1023)) {
  do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Error: " "input buffer too large\n", getpid(), gettid(), "inflate.c", 1068); } while (0);
  return (-4);
 }
 ...
 if (rc) {
  do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Error: " "inflate failed rc=%d\n", getpid(), gettid(), "inflate.c", 1081, rc); } while (0);
  return (-2);
 }
 ...
 if ((strm->dma_type[0] & DDCB_DMA_TYPE_MASK) ==
     DDCB_DMA_TYPE_FLAT)
  cmd->ats |= (((0x4ull) & 0xf) << (44 - (4 * (
# 1132 "inflate.c" 3 4
             __builtin_offsetof (
# 1132 "inflate.c"
             struct zedc_asiv_infl
...
# 1134 "inflate.c"
 ...
 if ((strm->dma_type[1] & DDCB_DMA_TYPE_MASK) ==
  DDCB_DMA_TYPE_FLAT)
  cmd->ats |= (((0x5ull) & 0xf) << (44 - (4 * (
# 1140 "inflate.c" 3 4
             __builtin_offsetof (
# 1140 "inflate.c"
             struct zedc_asiv_infl
...
# 1142 "inflate.c"
 ...
 if ((strm->dma_type[2] & DDCB_DMA_TYPE_MASK) ==
     DDCB_DMA_TYPE_FLAT) {
  cmd->ats |= ((((0x4ull) & 0xf) << (44 - (4 * (
# 1148 "inflate.c" 3 4
              __builtin_offsetof (
...
# 1180 "inflate.c"
 ...
 if ((strm->flags & (1 << 2)) &&
     (flush == 4) && (strm->avail_out > strm->avail_in * 2)) {
 ...
 for (i = 0; i < tries; i++) {
  ...
  if ((rc == -411) &&
      (cmd->retc == 0x0104) && (cmd->attn == 0x801A)) {
   strm->adler32 = strm->dict_adler32;
   do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Error: " "inflate ZEDC_NEED_DICT\n", getpid(), gettid(), "inflate.c", 1219); } while (0);
   return 2;
  }
# 1235 "inflate.c"
  if ((rc < 0) || (cmd->retc != 0x0102)) {
...
# 1238 "inflate.c"
   ,
                                         ...
                                         1241
# 1238 "inflate.c"
   , rc, cmd->retc, cmd->attn, cmd->progress, cmd->retc == 0x102 ? "" : "ERR"); } while (0)
  ...
  if ((strm->flags & (1 << 2)) &&
   ...
   do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Warn: " "[%s] What a pity, we guessed wrong " "and need to repeat\n", getpid(), gettid(),
                                    "inflate.c"
# 1257 "inflate.c"
   ,
                                    1258
# 1257 "inflate.c"
 ...
 if (rc < 0) {
  do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Error: " "inflate scratch update failed rc=%d\n", getpid(), gettid(), "inflate.c", 1265, rc); } while (0);
  return (-2);
 }
 ...
 if (strm->outp_returned > strm->avail_out) {
  do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Error: " "OUTP_RETURNED too large (0x%x)\n", getpid(), gettid(),
                        "inflate.c"
# 1272 "inflate.c"
  ,
                        1273
# 1272 "inflate.c"
  , strm->outp_returned); } while (0)
                         ;
  return (-2);
 }
 ...
 len = strm->inp_data_offs;



 if (len > strm->avail_in) {
  do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Error: " "consumed=%u/avail_in=%u\n", getpid(), gettid(), "inflate.c", 1288, len, strm->avail_in); } while (0);
  goto abort;
 }
 ...
 if (strm->avail_in && strm->avail_out) {
  do { if (zedc_log) fprintf(zedc_log, "%08x.%08x %s:%u: Warn: " "[%s] input not completely processed " "(avail_in=%d avail_out=%d zrc=%d)\n", getpid(), gettid(),
...
# 1317 "inflate.c"
  ,
                                                 ...
                                                 1319
# 1317 "inflate.c"
  , __func__, strm->avail_in, strm->avail_out, zrc); } while (0)
