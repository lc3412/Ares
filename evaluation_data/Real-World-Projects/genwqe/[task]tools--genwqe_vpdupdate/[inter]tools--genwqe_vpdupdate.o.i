...
typedef int error_t;
# 30 "genwqe_vpdupdate.c" 2

# 1 "../include/libcard.h" 1
# 42 "../include/libcard.h"
...
# 44 "../include/libcard.h" 2
# 95 "../include/libcard.h"
...
# 33 "genwqe_vpdupdate.c" 2
# 1 "../include/genwqe_vpd.h" 1
# 43 "../include/genwqe_vpd.h"
struct vpd_layout {
    const char *label;
    const int length;
    const char *mode;
...
# 89 "../include/genwqe_vpd.h" 3 4
_Bool 
# 89 "../include/genwqe_vpd.h"
    bin_2_csv(FILE *op, int fs, uint8_t *buffer);

# 90 "../include/genwqe_vpd.h" 3 4
_Bool 
# 90 "../include/genwqe_vpd.h"
    csv_2_bin(FILE *ip, uint8_t *buffer, int *size,
                    uint32_t *crc32_result,
                    uint32_t *crc32_from_csv);
# 34 "genwqe_vpdupdate.c" 2
...
{
 fprintf(
# 47 "genwqe_vpdupdate.c" 3 4
        stdout
# 47 "genwqe_vpdupdate.c"
              , "Usage: %s [OPTIONS]...\n"
  "\n"
...
# 78 "genwqe_vpdupdate.c" 3 4
_Bool 
# 78 "genwqe_vpdupdate.c"
     bin_ok = 
# 78 "genwqe_vpdupdate.c" 3 4
              0
# 78 "genwqe_vpdupdate.c"
                   ;
 int rc = 
# 79 "genwqe_vpdupdate.c" 3 4
         1
# 79 "genwqe_vpdupdate.c"
                     ;
 genwqe_vpd vpd;
 uint32_t crc = 0;
 ...
 if (0 == rc) {
  if (dump_level)
   genwqe_hexdump(fp, (uint8_t *)&vpd, (2*(sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + (sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + (sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + 2+7+7+7+13+5+4+6+6+4));

  do { if (verbose_flag) fprintf(
# 88 "genwqe_vpdupdate.c" 3 4
 stdout
# 88 "genwqe_vpdupdate.c"
 , "Checking now Binary VPD data from Card\n"); } while (0);
  crc = genwqe_crc32_gen((uint8_t *)&vpd, (2*(sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + (sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + (sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + 2+7+7+7+13+5+4+6+6+4), 0xffffffff);
  if (0 == crc)
   do { if (verbose_flag) fprintf(
# 91 "genwqe_vpdupdate.c" 3 4
  stdout
# 91 "genwqe_vpdupdate.c"
  , "Found Good VPD CRC\n"); } while (0);
  else do { fprintf(
# 92 "genwqe_vpdupdate.c" 3 4
      stderr
# 92 "genwqe_vpdupdate.c"
      , "%s:%u: Error: " "Wrong CRC in VPD 0x%x\n", "genwqe_vpdupdate.c", 92, crc); } while (0);
  do { if (verbose_flag) fprintf(
# 93 "genwqe_vpdupdate.c" 3 4
 stdout
# 93 "genwqe_vpdupdate.c"
 , "Display VPD data from Card\n"); } while (0);
  bin_ok = bin_2_csv(fp, (2*(sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + (sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + (sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + 2+7+7+7+13+5+4+6+6+4), (uint8_t *)&vpd);
  if (bin_ok)
   rc = 
# 96 "genwqe_vpdupdate.c" 3 4
       0
# 96 "genwqe_vpdupdate.c"
                   ;
  else do { fprintf(
# 97 "genwqe_vpdupdate.c" 3 4
      stderr
# 97 "genwqe_vpdupdate.c"
      , "%s:%u: Error: " "Invalid VPD. Use -dd option to dump data.\n", "genwqe_vpdupdate.c", 97); } while (0);
 } else do { fprintf(
# 98 "genwqe_vpdupdate.c" 3 4
       stderr
# 98 "genwqe_vpdupdate.c"
       , "%s:%u: Error: " "Faild to read VPD from Card (%d). Check -C option.\n", "genwqe_vpdupdate.c", 98, rc); } while (0);
 return rc;
}
...
# 104 "genwqe_vpdupdate.c" 3 4
_Bool 
# 104 "genwqe_vpdupdate.c"
     bin_ok = 
# 104 "genwqe_vpdupdate.c" 3 4
              0
# 104 "genwqe_vpdupdate.c"
                   ;
 int rc = 
# 105 "genwqe_vpdupdate.c" 3 4
         1
# 105 "genwqe_vpdupdate.c"
                     ;
 genwqe_vpd vpd;
 int n;
 uint32_t crc = 0;

 n = fread((uint8_t *)&vpd, 1 , (2*(sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + (sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + (sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + 2+7+7+7+13+5+4+6+6+4), fp);
 if ((2*(sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + (sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + (sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + 2+7+7+7+13+5+4+6+6+4) == n) {
  crc = genwqe_crc32_gen((uint8_t *)&vpd, (2*(sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + (sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + (sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + 2+7+7+7+13+5+4+6+6+4), 0xffffffff);
  if (0 == crc) {
   do { if (_dbg_flag) fprintf(
# 114 "genwqe_vpdupdate.c" 3 4
  stdout
# 114 "genwqe_vpdupdate.c"
  , "Input data CRC OK, Updating Card Now.\n"); } while(0);
   bin_ok = bin_2_csv(
# 115 "genwqe_vpdupdate.c" 3 4
                     stdout
# 115 "genwqe_vpdupdate.c"
                           , (2*(sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + (sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + (sizeof(vpd_ref_102)/sizeof(struct vpd_layout)) + 2+7+7+7+13+5+4+6+6+4), (uint8_t *)&vpd);
   if (bin_ok) {
    rc = genwqe_write_vpd(card, &vpd);
    if (rc == 0)
     rc = 
# 119 "genwqe_vpdupdate.c" 3 4
         0
# 119 "genwqe_vpdupdate.c"
                     ;
   } else do { fprintf(
# 120 "genwqe_vpdupdate.c" 3 4
         stderr
# 120 "genwqe_vpdupdate.c"
         , "%s:%u: Error: " "Invalid input file. Use -v option.\n", "genwqe_vpdupdate.c", 120); } while (0);
  } else do { fprintf(
# 121 "genwqe_vpdupdate.c" 3 4
        stderr
# 121 "genwqe_vpdupdate.c"
        , "%s:%u: Error: " "Invalid CRC: 0x%x in input file.\n", "genwqe_vpdupdate.c", 121, crc); } while (0);
 } else do { fprintf(
# 122 "genwqe_vpdupdate.c" 3 4
       stderr
# 122 "genwqe_vpdupdate.c"
       , "%s:%u: Error: " "%s\n", "genwqe_vpdupdate.c", 122, strerror(
# 122 "genwqe_vpdupdate.c" 3 4
       (*__errno_location ())
# 122 "genwqe_vpdupdate.c"
       )); } while (0);
...
static struct option long_options[] = {
 ...
 { "read", 
# 128 "genwqe_vpdupdate.c" 3 4
          0
# 128 "genwqe_vpdupdate.c"
                     , 
# 128 "genwqe_vpdupdate.c" 3 4
                          ((void *)0)
# 128 "genwqe_vpdupdate.c"
                              , 'r' },
 { "dump", 
# 129 "genwqe_vpdupdate.c" 3 4
          0
# 129 "genwqe_vpdupdate.c"
                     , 
# 129 "genwqe_vpdupdate.c" 3 4
                          ((void *)0)
# 129 "genwqe_vpdupdate.c"
                              , 'd' },
 { "update", 
# 130 "genwqe_vpdupdate.c" 3 4
            0
# 130 "genwqe_vpdupdate.c"
                       , 
# 130 "genwqe_vpdupdate.c" 3 4
                            ((void *)0)
# 130 "genwqe_vpdupdate.c"
                                , 'u' },
 { "show", 
# 131 "genwqe_vpdupdate.c" 3 4
          0
# 131 "genwqe_vpdupdate.c"
                     , 
# 131 "genwqe_vpdupdate.c" 3 4
                          ((void *)0)
# 131 "genwqe_vpdupdate.c"
 ...
 { "file", 
# 133 "genwqe_vpdupdate.c" 3 4
          1
# 133 "genwqe_vpdupdate.c"
                           , 
# 133 "genwqe_vpdupdate.c" 3 4
                             ((void *)0)
# 133 "genwqe_vpdupdate.c"
                                 , 'f' },
 { "card", 
# 134 "genwqe_vpdupdate.c" 3 4
          1
# 134 "genwqe_vpdupdate.c"
                           , 
# 134 "genwqe_vpdupdate.c" 3 4
                             ((void *)0)
# 134 "genwqe_vpdupdate.c"
                                 , 'C' },
 { "version", 
# 135 "genwqe_vpdupdate.c" 3 4
             0
# 135 "genwqe_vpdupdate.c"
                        , 
# 135 "genwqe_vpdupdate.c" 3 4
                             ((void *)0)
# 135 "genwqe_vpdupdate.c"
                                 , 'V' },
 { "verbose", 
# 136 "genwqe_vpdupdate.c" 3 4
             0
# 136 "genwqe_vpdupdate.c"
                        , 
# 136 "genwqe_vpdupdate.c" 3 4
                             ((void *)0)
# 136 "genwqe_vpdupdate.c"
                                 , 'v' },
 { "help", 
# 137 "genwqe_vpdupdate.c" 3 4
          0
# 137 "genwqe_vpdupdate.c"
                     , 
# 137 "genwqe_vpdupdate.c" 3 4
                          ((void *)0)
# 137 "genwqe_vpdupdate.c"
                              , 'h' },
 { 0, 
# 138 "genwqe_vpdupdate.c" 3 4
      0
# 138 "genwqe_vpdupdate.c"
                 , 
# 138 "genwqe_vpdupdate.c" 3 4
                      ((void *)0)
# 138 "genwqe_vpdupdate.c"
...
int main(int argc, char *argv[])
{
 ...
 char *env;
 char *fname = 
# 152 "genwqe_vpdupdate.c" 3 4
              ((void *)0)
# 152 "genwqe_vpdupdate.c"
                  ;
 FILE *fp_in = 
# 153 "genwqe_vpdupdate.c" 3 4
              ((void *)0)
# 153 "genwqe_vpdupdate.c"
                  ;
 FILE *fp_out = 
# 154 "genwqe_vpdupdate.c" 3 4
               ((void *)0)
# 154 "genwqe_vpdupdate.c"
                   ;
...
# 168 "genwqe_vpdupdate.c" 3 4
                                    ((void *)0)
# 168 "genwqe_vpdupdate.c"
                                        , 0);
   break;
  case 'f':
   fname = optarg;
  ...
  case 'h':
   usage(argv[0]);
   exit(
# 181 "genwqe_vpdupdate.c" 3 4
       0
# 181 "genwqe_vpdupdate.c"
                   );
   break;
  case 'V':
   fprintf(
# 184 "genwqe_vpdupdate.c" 3 4
          stdout
# 184 "genwqe_vpdupdate.c"
                , "%s\n", version);
   exit(
# 185 "genwqe_vpdupdate.c" 3 4
       0
# 185 "genwqe_vpdupdate.c"
                   );
  case 'v':
   verbose_flag++;
   break;
  ...
  default:
   usage(argv[0]);
   exit(
# 194 "genwqe_vpdupdate.c" 3 4
       1
# 194 "genwqe_vpdupdate.c"
                   );
  }
 }
 ...
 if (update_vpd && show_vpd) {
  fprintf(
# 199 "genwqe_vpdupdate.c" 3 4
         stderr
# 199 "genwqe_vpdupdate.c"
               , "Please give only -u or -s Option.\n");
  exit(
# 200 "genwqe_vpdupdate.c" 3 4
      1
# 200 "genwqe_vpdupdate.c"
                  );
 }

 if (update_vpd) {
  if (
# 204 "genwqe_vpdupdate.c" 3 4
     ((void *)0) 
# 204 "genwqe_vpdupdate.c"
          == fname) {
   fp_in = 
# 205 "genwqe_vpdupdate.c" 3 4
          stdin
# 205 "genwqe_vpdupdate.c"
               ;
   do { if (verbose_flag) fprintf(
# 206 "genwqe_vpdupdate.c" 3 4
  stdout
# 206 "genwqe_vpdupdate.c"
  , "Input from stdin.\n"); } while (0);
  } else {
   do { if (verbose_flag) fprintf(
# 208 "genwqe_vpdupdate.c" 3 4
  stdout
# 208 "genwqe_vpdupdate.c"
  , "Input File: <%s>\n", fname); } while (0);
   fp_in = fopen(fname, "r");
   if (
# 210 "genwqe_vpdupdate.c" 3 4
      ((void *)0) 
# 210 "genwqe_vpdupdate.c"
           == fp_in) {
    do { fprintf(
# 211 "genwqe_vpdupdate.c" 3 4
   stderr
# 211 "genwqe_vpdupdate.c"
   , "%s:%u: Error: " "%s Open Errno: <%s>\n",
                                 "genwqe_vpdupdate.c"
# 211 "genwqe_vpdupdate.c"
    ,
                                 212
...
# 211 "genwqe_vpdupdate.c"
                                  ...
                                  ;
    exit (
# 213 "genwqe_vpdupdate.c" 3 4
         1
# 213 "genwqe_vpdupdate.c"
                     );
   }
   fp_out = 
# 215 "genwqe_vpdupdate.c" 3 4
           stdout
# 215 "genwqe_vpdupdate.c"
                 ;
  }
 }
 if (show_vpd) {
  if (
# 219 "genwqe_vpdupdate.c" 3 4
     ((void *)0) 
# 219 "genwqe_vpdupdate.c"
          == fname) {
   fp_out = 
# 220 "genwqe_vpdupdate.c" 3 4
           stdout
# 220 "genwqe_vpdupdate.c"
                 ;
   do { if (verbose_flag) fprintf(
# 221 "genwqe_vpdupdate.c" 3 4
  stdout
# 221 "genwqe_vpdupdate.c"
  , "Output to stdout.\n"); } while (0);
  } else {
   fp_out = fopen(fname, "w");
   if (
# 224 "genwqe_vpdupdate.c" 3 4
      ((void *)0) 
# 224 "genwqe_vpdupdate.c"
           == fp_out) {
    do { fprintf(
# 225 "genwqe_vpdupdate.c" 3 4
   stderr
# 225 "genwqe_vpdupdate.c"
   , "%s:%u: Error: " "%s Open Err: <%s>\n",
                                 "genwqe_vpdupdate.c"
# 225 "genwqe_vpdupdate.c"
    ,
                                 226
...
# 225 "genwqe_vpdupdate.c"
                                  ...
                                  ;
    exit (
# 227 "genwqe_vpdupdate.c" 3 4
         1
# 227 "genwqe_vpdupdate.c"
                     );
   }
  }
 }
 if ((
# 231 "genwqe_vpdupdate.c" 3 4
     ((void *)0) 
# 231 "genwqe_vpdupdate.c"
          == fp_in) && (
# 231 "genwqe_vpdupdate.c" 3 4
                        ((void *)0) 
# 231 "genwqe_vpdupdate.c"
                             == fp_out)) {
  fprintf(
# 232 "genwqe_vpdupdate.c" 3 4
         stderr
# 232 "genwqe_vpdupdate.c"
               , "Please give -u or -s Option\n");
  exit(
# 233 "genwqe_vpdupdate.c" 3 4
      1
# 233 "genwqe_vpdupdate.c"
                  );
 }
 ...
 if ((env) && (atoi(env) > 0)) {
  do { fprintf(
# 239 "genwqe_vpdupdate.c" 3 4
 stderr
# 239 "genwqe_vpdupdate.c"
 , "%s:%u: Error: " "driver / HW simulation active !\n", "genwqe_vpdupdate.c", 239); } while (0);
  if (show_vpd && fname) fclose(fp_out);
  if (update_vpd && fname) fclose(fp_in);
  exit(
# 242 "genwqe_vpdupdate.c" 3 4
      1
# 242 "genwqe_vpdupdate.c"
                  );
 }
 ...
 if (-1 == card_no) {
  do { fprintf(
# 247 "genwqe_vpdupdate.c" 3 4
 stderr
# 247 "genwqe_vpdupdate.c"
 , "%s:%u: Error: " "Specify a valid GENWQE Card number (e.g. -C 0)\n", "genwqe_vpdupdate.c", 247); } while (0);
  if (show_vpd && fname) fclose(fp_out);
  if (update_vpd && fname) fclose(fp_in);
  exit(
# 250 "genwqe_vpdupdate.c" 3 4
      1
# 250 "genwqe_vpdupdate.c"
                  );
 }
 do { if (verbose_flag) fprintf(
# 252 "genwqe_vpdupdate.c" 3 4
stdout
# 252 "genwqe_vpdupdate.c"
, "Try to open Card: %d\n", card_no); } while (0);
 card = genwqe_card_open(card_no, 0x0004, &err_code,
    0, 0x0000000000000000);
 if (
# 255 "genwqe_vpdupdate.c" 3 4
    ((void *)0) 
# 255 "genwqe_vpdupdate.c"
         == card) {
  do { fprintf(
# 256 "genwqe_vpdupdate.c" 3 4
 stderr
# 256 "genwqe_vpdupdate.c"
 , "%s:%u: Error: " "cannot open Genwqe Card: %d (err: %d)\n",
                          "genwqe_vpdupdate.c"
# 256 "genwqe_vpdupdate.c"
  ,
                          257
# 256 "genwqe_vpdupdate.c"
  , card_no, err_code); } while (0)
                           ;
  if (show_vpd && fname) fclose(fp_out);
  if (update_vpd && fname) fclose(fp_in);
  exit(
# 260 "genwqe_vpdupdate.c" 3 4
      1
# 260 "genwqe_vpdupdate.c"
                  );
 }
